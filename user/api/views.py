...
class usersNew(APIView):
    def post(self,request):
        ...
        if serializer.is_valid():
            ...
            imageupdate=image.objects.filter(user=serializer.data['id']).update(qrprofile=path+fil)
            credits=Credits.objects.create(number_credits=0,user_id=serializer.data['id'])
...
class LastEmployee(APIView):
    def get(self,request):
        ...
        users=User.objects.filter(id=imagen.user_id).values('id','first_name','last_name','username','email','date_joined').first()
...
class UserProfile(APIView):
    ...
    def get(self,request):
        ...
        for i in image.objects.filter(user=request.user.id):
            print(i.user_id)
            users=User.objects.filter(id=i.user_id).values('id','first_name','last_name','username','email')
            credit=Credits.objects.filter(user_id=i.user_id).values('id','number_credits')
            count_movies=Ticket.objects.filter(credits_ticket_id=credit[0]["id"]).count()
            count_survey=Survey.objects.filter(status=2,user_id=request.user.id).count()
            print(users)
            datas={
                'id':users[0]['id'],
